...
export default class ProfileView extends View {
  ...
  private populateAddresses(customerData: Customer | null): void {
    ...
    if (customerData && customerData.addresses) {
      ...
      if (addresses.length === 1) {
        // Если только один адрес, используем его для обоих контейнеров
        ...
        // this.form.node.appendChild(this.billingAndShipping.htmlElement);
      } else {
        // Если два адреса, размещаем их в соответствующих контейнерах
  ...
  private addCountryInput(address: Address, container: FormSectionView) {
    const attrs: IFormInputField = {
      ...
      disabled: true,
      customValidators: [], // Добавить кастомные или переделать
    };
