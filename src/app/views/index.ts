// views/index.ts
export { default as Error404 } from './404/404';
export { default as CatalogPageView } from './catalog/catalog-page';
export { default as FooterView } from './footer/footer';
export { default as HeaderView } from './header/header';
export { default as LoginView } from './login/login-page';
export { default as MainView } from './main-view';
export { default as MainPageView } from './main/main-page';
export { default as ProfileView } from './profile/profile-page';
export { default as ProductPageView } from './product/product-page';
export { default as RegistrationView } from './registration/registration-page';
